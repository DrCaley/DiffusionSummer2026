"""
Train the white-noise DDPM model.

Noise schedule : linear (beta_1=1e-4 → beta_T=0.02, hardcoded)
Noise type     : white — i.i.d. Gaussian (flat spectrum)
...
Usage (run from workspace root, with data.pickle present):
    python "Colored Noise Test/white_noise/train.py" --resume "Colored Noise Test/white_noise/checkpoints/best_model.pt"
    python "Colored Noise Test/white_noise/train.py" --resume ... --patience 30 --epochs 500
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Train white-noise DDPM (linear schedule).")
