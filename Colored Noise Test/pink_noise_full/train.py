"""
Train the pink-noise-full DDPM model.

This variant uses pink (1/f) noise throughout the entire diffusion chain —
forward process, reverse denoising steps, and RePaint resampling all use
spatially-correlated pink noise.  No white noise is used anywhere.

Noise schedule : linear (beta_1=1e-4 → beta_T=0.02, hardcoded)
Noise type     : pink_full — 1/f spectrum used for all noise injections
...
Usage (run from workspace root, with data.pickle present):
    python "Colored Noise Test/pink_noise_full/train.py" --pickle data.pickle
    python "Colored Noise Test/pink_noise_full/train.py" --resume "Colored Noise Test/pink_noise_full/checkpoints/best_model.pt"
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Train pink-noise-full DDPM (linear schedule, all-colored noise).")
