"""
Train the red-noise DDPM model.

Noise schedule : linear (beta_1=1e-4 → beta_T=0.02, hardcoded)
Noise type     : red (Brownian) — 1/f² spectrum (smooth, low-frequency emphasis)
...
Usage (run from workspace root, with data.pickle present):
    python "Colored Noise Test/red_noise/train.py" --resume "Colored Noise Test/red_noise/checkpoints/best_model.pt"
    python "Colored Noise Test/red_noise/train.py" --resume ... --patience 30 --epochs 500
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Train red-noise DDPM (linear schedule).")
