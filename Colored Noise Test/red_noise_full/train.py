"""
Train the red-noise-full DDPM model.

This variant uses red (Brownian, 1/f²) noise throughout the entire diffusion
chain — forward process, reverse denoising steps, and RePaint resampling all
use spatially-correlated red noise.  No white noise is used anywhere.

Noise schedule : linear (beta_1=1e-4 → beta_T=0.02, hardcoded)
Noise type     : red_full — 1/f² spectrum used for all noise injections
...
Saved checkpoints:
    ...
    checkpoints/ckpt_epochNNNN.pt      — saved every --save_every epochs (default 100)

Usage (run from workspace root, with data.pickle present):
    python "Colored Noise Test/red_noise_full/train.py" --pickle data.pickle
    python "Colored Noise Test/red_noise_full/train.py" --resume "Colored Noise Test/red_noise_full/checkpoints/best_model.pt"
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Train red-noise-full DDPM (linear schedule, all-colored noise).")
