"""
Train the annealed-noise DDPM model.

Noise schedule : linear (beta_1=1e-4 → beta_T=0.02)
Noise type     : annealed — α(t) = 2·t/T
                 t=T → red (smooth, large-scale structure destroyed first)
                 t=0 → white (fine detail destroyed last)

Early stopping : stops when val loss does not improve by --min_delta over
                 --patience consecutive epochs.
Saved checkpoints:
    checkpoints/best_model.pt          — best validation loss
    checkpoints/ckpt_epochNNNN.pt      — every --save_every epochs (default 100)

Usage (run from workspace root, with data.pickle present):
    python "Colored Noise Test/annealed_noise/train.py" --pickle data.pickle
    python "Colored Noise Test/annealed_noise/train.py" --resume "Colored Noise Test/annealed_noise/checkpoints/best_model.pt"
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Train annealed-noise DDPM.")
