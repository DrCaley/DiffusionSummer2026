"""
batch_white_vs_red.py  –  White vs Red RePaint comparison.

Runs RePaint for white_noise and red_noise models across N seeds and
saves a 1×3 composite PNG (GT | White | Red) plus a summary.txt.

Usage:
    python "Colored Noise Test/batch_white_vs_red.py" \\
        --pickle  /root/model_pink_noise/data.pickle \\
        --ckpt    best          # or epoch100 / best_by_100
        --out_dir "Colored Noise Test/outputs/white_vs_red_r1"
"""
...
# ── Custom repaint (copied from batch_infer_all.py) ──────────────────────────
...
def main():
    ...
    out_dir = args.out_dir or os.path.join(_HERE, "outputs", "white_vs_red_r1")
    ...
    for i in range(args.n_samples):
        ...
        # ── Figure: 1 row × 3 cols (GT | White | Red) ────────────────────────
        fig, axes = plt.subplots(1, 3, figsize=(21, 7))
        fig.suptitle(
            f"White vs Red — sample {i}, seed {sample_seed}\n"
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    lines = [
        f"White vs Red RePaint comparison",
    ...
    header = f"  {'sample':>6}  " + "  ".join(f"{label[:8]:>8}" for _, label in MODELS)
    lines.append(header)
    for i in range(args.n_samples):
