"""
DDPM diffusion utilities for the blue-noise experiment.

Noise schedule : linear (beta_1=1e-4 → beta_T=0.02)
Noise type     : blue — S(f) ∝ f² spectrum (high-frequency emphasis).
                 Opposite of red noise; suppresses large-scale correlated
                 structure and emphasises fine-scale variation.
                 Generated by filtering white noise with amplitude ∝ f in
                 the 2-D Fourier domain, then normalising to noise_std.
...
class DDPM:
    """
    DDPM with a linear beta schedule and blue noise.

    Beta schedule : linear, beta_1=1e-4 → beta_T=0.02
    Noise type    : blue — spectral amplitude ∝ f → power ∝ f²
    Loss          : eps-MSE + CURL_DIV_WEIGHT * curl_div_loss
    """
    ...
    def _make_noise(self, x0: torch.Tensor) -> torch.Tensor:
        """
        Blue noise: S(f) ∝ f² spectrum (high-frequency emphasis).

        Generates white noise, filters in the frequency domain with
        amplitude ∝ f (so power ∝ f²), zeros the DC component to
        prevent mean drift, then normalises to noise_std.
        """
        B, C, H, W = x0.shape
        device = x0.device
        white  = torch.randn(B, C, H, W, device=device)

        # 2-D radial frequency grid
        ...
        # Blue filter: amplitude ∝ f  →  power ∝ f²
        filt = f_radial.clone()
        filt[f_radial == 0] = 0.0           # zero DC (no mean offset)
        filt = filt[None, None]             # (1, 1, H, W)

        noise_fft = torch.fft.fft2(white)
        noise_fft = noise_fft * filt
        colored   = torch.fft.ifft2(noise_fft).real
