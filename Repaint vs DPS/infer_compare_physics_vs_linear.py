"""
infer_compare_physics_vs_linear.py
===================================
Head-to-head comparison of two Repaint-UNet checkpoints under Diffusion
Posterior Sampling (DPS, zeta=0.04) inference:
...
Both models share the same Repaint UNet architecture, so a single DPS
inference routine (dps_infer, ported from infer_compare_unet_fno.py) is used
for both. For N seeds using biased_walk_path (random-walk observation path),
each seed's sample index and path mask are shared across both models
(paired comparison). Reports RMSE, magnitude error, and angle error per
model, averaged over ocean pixels and seeds.

Saves results.csv, summary.txt, and bar_chart.png.

Usage:
    python infer_compare_physics_vs_linear.py \\
        ...
        --out_dir /workspace/DiffusionSummer2026/RepaintVsDPS/results/physics_vs_linear_dps004_s10_50seeds \\
        --n_seeds 50 --T 1000 --stride 10 --step_size 0.04 --path_steps 150
"""
...
def save_bar_chart(all_rmse, all_mag, all_ang, all_times, T, stride, step_size, n_seeds, out_path):
    ...
    fig.suptitle(f"Physics-Informed vs Linear-CurlDiv  —  DPS zeta={step_size}  "
...
def main():
    ...
    p.add_argument("--out_dir",             default="compare_physics_vs_linear")
    ...
    for run_i, seed in enumerate(seeds):
        ...
        for model_name in MODEL_NAMES:
            ...
            pred = dps_infer(model, diffusion, x0_obs, path_mask, land_mask,
                              device=device, stride=args.stride, step_size=args.step_size)
    ...
    with open(summary_path, "w") as f:
        f.write(f"Physics-Informed vs Linear-CurlDiv  —  DPS zeta={args.step_size}  "
    ...
    save_bar_chart(all_rmse, all_mag, all_ang, all_times, args.T, args.stride,
                    args.step_size, n_total, chart_path)
