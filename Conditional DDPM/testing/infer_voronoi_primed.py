"""
Batch inference for the Voronoi-Primed Conditional DDPM on validation samples.

Conditioning: path_field  [u_path, v_path, path_mask]
Inference:    RePaint initialised from a noised Voronoi tessellation instead of
              pure Gaussian noise, using CondDDPMVP.repaint.

Usage (run from workspace root):
    py "Conditional DDPM/infer_voronoi_primed.py"
    py "Conditional DDPM/infer_voronoi_primed.py" --n_samples 10 --seed 0
    py "Conditional DDPM/infer_voronoi_primed.py" --checkpoint path/to/ckpt.pt
"""
...
# ---------------------------------------------------------------------------
# Build conditioning + Voronoi for a single sample
# ---------------------------------------------------------------------------

@torch.no_grad()
def make_cond_and_voronoi_single(
    x0:            torch.Tensor,   # (2, H, W) clean field on device
    ...
    Returns:
        ...
        path_mask: (H, W) bool numpy array
        x_voronoi: (1, 2, H, W) Voronoi u, v channels
    """
    C, H, W = x0.shape
    ...
    # Voronoi tessellation from the same path
    rows_n = (
    ...
    x_voronoi    = voronoi_full[:, :C, :, :]                          # (1, 2, H, W)

    return cond, path_mask, x_voronoi
...
def save_sample_plot(
    ...
    """2×3 layout: GT | Path | Voronoi  /  Prediction | Error | (blank)"""
    fig, axes = plt.subplots(2, 3, figsize=(24, 10))
    fig.suptitle(
        f"VP Cond-DDPM (path_field)  |  val sample {sample_idx}  |  seed {seed}\n"
        f"RMSE={rmse:.4f}",
        fontsize=13,
    )
    ...
    axes[0, 1].set_title(f"Robot Path ({path_mask.sum()} cells)", fontsize=11)
    ...
    plot_field(axes[0, 2], u_vor, v_vor, land_mask, "Voronoi Tessellation", cmap="viridis")

    # Row 1
    plot_field(axes[1, 0], u_pred, v_pred, land_mask, "VP Prediction", cmap="cool")
    ...
    plt.colorbar(im, ax=axes[1, 1], label="Speed error", shrink=0.7)
    axes[1, 1].set_title(f"Error  (RMSE={rmse:.4f})", fontsize=11)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Inference for the Voronoi-Primed Conditional DDPM."
    )
    ...
    p.add_argument("--checkpoint", default=None,
                   help="Path to .pt checkpoint. Defaults to "
                        ...
                        "best_cond_ddpm_vp_path_field_cosine.pt'")
    ...
    p.add_argument("--out_dir",    default=None,
                   help="Output directory. Defaults to "
                        "'Conditional DDPM/results_vp_path_field/'.")
    p.add_argument("--resample",   type=int, default=10,
                   help="RePaint resampling iterations per timestep (default: 10).")
    return p.parse_args()
...
def main():
    ...
    if args.checkpoint is None:
        args.checkpoint = os.path.join(
            _here, "checkpoints_vp_path_field",
            "best_cond_ddpm_vp_path_field_cosine.pt",
        )
    if args.out_dir is None:
        args.out_dir = os.path.join(_here, "results_vp_path_field")
    os.makedirs(args.out_dir, exist_ok=True)

    print(f"Device     : {device}")
    print(f"Cond mode  : path_field (VP)")
    ...
    for run, sample_idx in enumerate(sample_idxs, start=1):
        ...
        cond, path_mask, x_voronoi = make_cond_and_voronoi_single(
            x0_true, land_mask_np, voronoi_layer,
            args.path_steps, seed, device,
        )

        path_t       = torch.from_numpy(path_mask.astype(np.float32)).to(device)
        x0_known_b   = (x0_true * path_t.unsqueeze(0)).unsqueeze(0)  # (1, 2, H, W)
    ...
    ax.axhline(np.mean(rmses), color="red", linestyle="--",
    ...
    ax.set_ylabel("RMSE")
    ax.set_title("VP Cond-DDPM (path_field) — per-sample RMSE on val samples")
    ax.legend()
    plt.tight_layout()
